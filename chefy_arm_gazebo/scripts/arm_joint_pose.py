...
class ArmJointPose:
    def __init__(self):
        ...
        self.states_sub = rospy.Subscriber("/chefy_arm/set_joints_states", JointState, self.callback,
                                           queue_size=2)
        # a list of publisher
        self.joint_pub = dict()
        self.joint_pose = dict()
        for idx, name in enumerate(self.joint_names):
            pub = rospy.Publisher("/chefy_arm/joint%d_position_controller/command" % (idx + 1), Float64, queue_size=3)
